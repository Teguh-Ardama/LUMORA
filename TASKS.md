...
- [x] UI pilih sticker (multi-select, 0-N) — **StickerEditor component siap**
...
- [ ] **Keputusan MVP**: hanya wajah terbesar/dominan yang dipasangi sticker
...
- [ ] Mode "Custom": buka shared slider component dari Phase 3
...
- [ ] Scaffold `apps/desktop-bridge` (Tauri)
- [ ] GUI: pilih folder tethering, pilih event aktif, status koneksi (connected/disconnected)
- [ ] `chokidar` watch folder, debounce/stable check sebelum kirim (hindari partial file)
- [ ] Kompres lokal sebelum POST, retry exponential backoff (maks 3x) kalau gagal
- [ ] Auth: bridge token per-event (lihat `docs/SECURITY.md` §3)
