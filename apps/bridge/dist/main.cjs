...
var ConfigStore = class {
  ...
  set(patch) {
    ...
    (0, import_node_fs.mkdirSync)(import_node_path.default.dirname(this.filePath), { recursive: true });
    const tmp = `${this.filePath}.tmp`;
    (0, import_node_fs.writeFileSync)(tmp, JSON.stringify(this.cache, null, 2));
    (0, import_node_fs.writeFileSync)(this.filePath, JSON.stringify(this.cache, null, 2));
    try {
...
var buffer = [];
var win = null;
function attachLogWindow(window) {
  win = window;
  for (const line of buffer) window.webContents.send(IpcChannel.LOG, line);
}
function push(level, message) {
  const line = { ts: (/* @__PURE__ */ new Date()).toISOString(), level, message };
  buffer.push(line);
...
// ../../packages/image/src/server/compress.ts
var import_sharp = __toESM(require("sharp"), 1);
async function compressToTarget(input, opts) {
  const maxLongEdge = opts.maxLongEdge ?? 2400;
  const qualityLadder = [88, 82, 76, 70, 64];
  let longEdge = maxLongEdge;
  while (true) {
    const base = (0, import_sharp.default)(input, { failOn: "none" }).rotate().resize(longEdge, longEdge, { fit: "inside", withoutEnlargement: true });
    for (const quality of qualityLadder) {
      const out = await base.clone().jpeg({ quality, mozjpeg: true, chromaSubsampling: quality > 80 ? "4:4:4" : "4:2:0" }).toBuffer({ resolveWithObject: true });
...
var ALL_PERMISSIONS = Object.values(Permission);
var OPERATOR_PERMISSIONS = [
  Permission.ORG_READ,
...
var layoutConfigSchema = z.object({
  ...
  background: z.string().regex(/^#(?:[0-9a-fA-F]{3}){1,2}$/, "background must be a hex color").default("#ffffff"),
  ...
  for (const [i, slot] of cfg.slots.entries()) {
    ...
    if (slot.x + slot.w > maxX) {
      ctx.addIssue({
        ...
        path: ["slots", i],
        message: cfg.mode === LayoutMode.STRIP ? "STRIP slots must fit within the left half of the canvas" : "Slot exceeds canvas width"
      });
    }
...
var passwordSchema = z.string().min(10, "Password must be at least 10 characters").max(128).regex(/[a-z]/, "Must contain a lowercase letter").regex(/[A-Z]/, "Must contain an uppercase letter").regex(/[0-9]/, "Must contain a digit");
...
var acceptInviteSchema = z.object({
...
var updateOrganizationSchema = z.object({
  ...
  brandColor: z.string().regex(/^#(?:[0-9a-fA-F]{3}){1,2}$/).nullish()
});
...
var removeMemberSchema = z.object({
  userId: uuidSchema
});
...
var assignOperatorsSchema = z.object({
  userIds: z.array(uuidSchema).max(50)
});

// ../../packages/contracts/src/template.ts
var createBorderSchema = z.object({
  name: z.string().trim().min(2).max(255),
  eventId: uuidSchema.nullish()
});
var updateBorderSchema = z.object({
  name: z.string().trim().min(2).max(255).optional(),
  isActive: z.boolean().optional()
});
var createLayoutSchema = z.object({
  name: z.string().trim().min(2).max(100),
  eventId: uuidSchema.nullish(),
  config: layoutConfigSchema
});
var updateLayoutSchema = z.object({
...
var updatePrintJobSchema = z.object({
  status: z.enum(["PRINTING", "PRINTED", "FAILED", "CANCELLED"]),
  failureReason: z.string().max(500).optional()
});

// ../../packages/contracts/src/bridge.ts
var createPairingCodeSchema = z.object({
  eventId: uuidSchema
});
var pairBridgeSchema = z.object({
  code: z.string().trim().toUpperCase().regex(/^[A-Z0-9]{8}$/, "Pairing code is 8 alphanumeric characters"),
  deviceName: z.string().trim().min(2).max(120),
  platform: z.string().trim().max(60),
  appVersion: z.string().trim().max(30)
});
var bridgeHeartbeatSchema = z.object({
...
var auditQuerySchema = paginationQuerySchema.extend({
...
var OfflineQueue = class {
  items = [];
  journalPath = import_node_path2.default.join(import_electron2.app.getPath("userData"), "upload-queue.json");
  pumping = false;
  ...
  restore() {
    try {
      this.items = JSON.parse((0, import_node_fs2.readFileSync)(this.journalPath, "utf8"));
  ...
  persist() {
    (0, import_node_fs2.mkdirSync)(import_node_path2.default.dirname(this.journalPath), { recursive: true });
    (0, import_node_fs2.writeFileSync)(this.journalPath, JSON.stringify(this.items, null, 2));
  }
  get depth() {
  ...
  enqueue(filePath, sessionId, sequence) {
    const filename = import_node_path2.default.basename(filePath);
    const idempotencyKey = (0, import_node_crypto.createHash)("sha256").update(`${filePath}:${sessionId ?? "none"}`).digest("hex").slice(0, 48);
  ...
  async pump() {
    ...
    try {
      while (this.items.length > 0) {
        ...
        try {
          const raw = await (0, import_promises.readFile)(item.filePath);
          ...
          this.lastUploadAt = (/* @__PURE__ */ new Date()).toISOString();
          this.persist();
          this.notify();
          ...
          item.attempts += 1;
          this.persist();
          const delay = Math.min(6e4, 2e3 * 2 ** Math.min(item.attempts, 5));
          blog.warn(
            `Upload failed for ${item.filename} (attempt ${item.attempts}): ${err instanceof Error ? err.message : String(err)} \u2014 retrying in ${Math.round(delay / 1e3)}s`
          );
...
var PHOTO_EXTENSIONS = /* @__PURE__ */ new Set([".jpg", ".jpeg", ".png"]);
var TetherWatcher = class {
  ...
  start(folder) {
    ...
    this.watcher = import_chokidar.default.watch(folder, {
      ignoreInitial: true,
      // pre-existing files are not this event's shots
      depth: 2,
    ...
    this.watcher.on("add", (filePath) => {
      ...
      setTimeout(() => {
        if (this.active && Date.now() - this.startedAt > 5e3) this.start(folder);
      }, 3e3);
    });
  }
  stop() {
...
function broadcastState() {
  if (win2 && !win2.isDestroyed()) {
    win2.webContents.send(IpcChannel.STATE_CHANGED, currentState());
  }
}
...
function wireIpc() {
  import_electron3.ipcMain.handle(IpcChannel.GET_STATE, () => currentState());
  import_electron3.ipcMain.handle(IpcChannel.PAIR, async (_e, req) => {
    try {
      const result = await bridgeApi.pair(req.apiUrl, {
        code: req.code.trim().toUpperCase(),
        deviceName: req.deviceName.trim() || import_node_os.default.hostname(),
  ...
  import_electron3.ipcMain.handle(IpcChannel.UNPAIR, () => {
    watcher.stop();
    watcherActive = false;
  ...
  import_electron3.ipcMain.handle(IpcChannel.SELECT_FOLDER, async () => {
    if (!win2) return null;
  ...
  import_electron3.ipcMain.handle(IpcChannel.SET_WATCHER, (_e, active) => {
    const cfg = getConfigStore().get();
    if (active) {
      if (!cfg.watchFolder) {
...
function createWindow() {
  win2 = new import_electron3.BrowserWindow({
    width: 860,
    height: 640,
    minWidth: 720,
    minHeight: 520,
...
void import_electron3.app.whenReady().then(() => {
